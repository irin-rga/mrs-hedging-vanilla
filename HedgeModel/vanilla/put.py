from  HedgeModel.vanilla.blackscholes import BSPut
...
class Put(Vanilla):
    ...
    def calc(self, s: float, r: float, q: float, vol: float, t: float, calctype: CalcType = CalcType.Price):
        ...
        # Calc the result
        result_per_contract = self.position_mult * BSPut(s, self.strike, r, q, vol, t, str(calctype))
    ...
    def payoff(self, idx_lvl_exp_dt: float):
        return self.position_mult * max(0.0, self.strike - idx_lvl_exp_dt)